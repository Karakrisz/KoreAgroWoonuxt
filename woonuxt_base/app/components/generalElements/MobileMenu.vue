...
<template>
  <div
    class="bg-white flex flex-col max-w-lg shadow-lg top-0 bottom-0 left-0 w-11/12 z-50 fixed overflow-x-hidden p-20">
    <Icon name="ion:close-outline"
      ...
      size="34" @click="toggleMobileMenu(false)" />
    ...
    <div class="flex flex-col items-center justify-center h-full gap-10">
      ...
      <nav class="flex flex-col items-center justify-center gap-10">
        <NuxtLink v-for="link in links" :key="link.to" :to="link.to" :class="[
          ...
          link.class
        ]" active-class="text-primary" :prefetch="link.prefetch">
          {{ link.text }}
        </NuxtLink>
      </nav>
    </div>
